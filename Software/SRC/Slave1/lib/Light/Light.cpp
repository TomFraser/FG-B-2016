...
light::light(int pin1, int pin2, int pin3, int pin4, int pin5, int pin6, int pin7, int pin8, int pin9, int pin10, int pin11, int pin12, int pin13, int pin14, int pin15, int pin16, bool isGoing){ //The init function, sets the light sensor pins to inputs
    ...
    //Goal of this branch is to select one correct sensor for each arm then incude the same sensors for each arm. BECAUSE FUCKING LIGHT SENSORS BREAK ALOT! FIX IT TOM GOD

    LeftLight1Pin = pin1;   //WORKING   //left
    LeftLight2Pin = pin2;   //WORKING
    LeftLight3Pin = pin3;   //WORKING
    LeftLight4Pin = pin4;   //WORKING

    RightLight1Pin = pin5;  //WORKING //Right
    RightLight2Pin = pin6;  //WORKING
    RightLight3Pin = pin7;  //WORKING
    RightLight4Pin = pin8;  //WORKING

    BackLight1Pin = pin9;   //WORKING //Back
    BackLight2Pin = pin10;  //WORKING
    BackLight3Pin = pin11;  //WORKING
    BackLight4Pin = pin12;  //WORKING

    FrontLight1Pin = pin13; //WORKING //Front
    FrontLight2Pin = pin14; //WORKING
    FrontLight3Pin = pin15; //WORKING
    FrontLight4Pin = pin16; //WORKING
...
//God damn Im Stoopid //This crap was from when I was manually setting values to test that the code worked
    ...
    if(warningTSOP > 17 || warningTSOP < 3){ //checking if the tsop is between 17 and 3 which is the front of the robot
        ...
        for(int i = 4; i < 8; i++){
            if(finalLightData[i] >= 1){
                switch(i){
                    case 4: warningInt = -8; break; //Outermost
                    case 5: warningInt = -8; break;
                    case 6: warningInt = -8; break;
        ...
        for(int i = 8; i < 12; i++){
            if(finalLightData[i] >= 1){
                switch(i){
                    case 8: warningInt = -8; break; //Outermost
                    case 9: warningInt = -8; break;
                    case 10: warningInt = -8; break;
        ...
        for(int i = 12; i < 16; i++){
            if(finalLightData[i] >= 1){
                switch(i){
                    case 12: warningInt = -8; break; //Outermost
                    case 13: warningInt = -8; break;
                    case 14: warningInt = -8; break;
